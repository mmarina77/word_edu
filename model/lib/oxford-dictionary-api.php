...
class Dictionary {
	...
	*/
	...
	private function DecodeAPIRequest($result){
		...
		if( isset($this->jsonObject->results) ) {
		...
		/* Synonyms */
		if(isset($this->jsonObject->results[$this->resultSet]->lexicalEntries[0]->entries[0]->senses[0]->synonyms)){
			$synonyms = $this->jsonObject->results[$this->resultSet]->lexicalEntries[0]->entries[0]->senses[0]->synonyms;
			$arr = [];
			foreach($synonyms as $value) {
				$arr[] = $value->text;
			}
			$this->synonyms = sizeof($arr) > 0 ? $arr : null;
		} else {
			$this->synonyms = null;
		}
